...
void UpdateLocalPlaylist()
{
    ...
    if (!filesystem::exists(br_m3u8_fname) && !filesystem::exists(br_xml_fname))
    {
        cout << "Baixando arquivo de https://i.mjh.nz/PlutoTV" << endl;
    ...
    else
    {
        ...
        filesystem::remove(br_m3u8_fname.c_str());
        cout << "Removendo br.xml" << endl;
        filesystem::remove(br_xml_fname.c_str());
